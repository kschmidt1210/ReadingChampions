...
function StatCard({
  ...
  return (
    <div className="rounded-lg bg-gray-50 px-3 py-2.5 min-w-0">
      <div className="text-[0.65rem] font-semibold uppercase tracking-wide text-gray-400 mb-0.5">
        {label}
      </div>
      <div className="text-sm font-bold text-gray-800 tabular-nums">{value}</div>
      {sub && (
        <div className="text-[0.6rem] text-gray-400 mt-0.5 truncate">{sub}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex items-start gap-2 min-w-0">
      ...
      <div className="min-w-0">
        ...
        <div className="text-[0.65rem] text-gray-400">{detail}</div>
      </div>
    </div>
  );
}
